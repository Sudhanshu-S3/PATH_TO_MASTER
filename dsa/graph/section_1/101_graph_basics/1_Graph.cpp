...
public:
    ...
    void printGraph()
    {
        for (int i = 0; i < nodes; i++)
        {
            ...
            {
                cout << "From Node" << i << " to Node " << a.first << " weight :" << a.second
                     << endl;
            }
        }
    }
...
/*
    weighted graph => vector<vector<pair<int,int>>> graph(nodes, vector<int> (vertices attached ,
   weight of the edge))
...
/*
    ...
    For undirected connected graphs, a partial check is: If number of edges (E) > number of vertices
   (V) - 1, cycles exist.
